    ...
    <div class="nav-actions">
        <?php if ($isLoggedIn): ?>
            <div class="nav-user-info" style="display: flex; align-items: center; gap: 12px;">
                ...
                <span style="color: white; font-weight: 500;">Bonjour, <?= htmlspecialchars($userNom) ?></span>
...
<section class="section-padding">
    ...
    <p class="section-desc">
        ...
        <!-- Barre de tri améliorée -->
<div class="filter-bar" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 30px; flex-wrap: wrap; gap: 15px;">
    <div style="display: flex; gap: 10px; align-items: center;">
       
        
        <!-- Bouton Trier moderne -->
        <div class="sort-menu" style="position: relative; display: inline-block;">
            <button class="btn-sort" onclick="toggleSortMenu()" 
                    style="background: white; border: 2px solid #e2e8f0; border-radius: 40px; padding: 10px 20px; font-size: 0.9rem; font-weight: 500; color: #1e293b; cursor: pointer; display: inline-flex; align-items: center; gap: 10px; transition: all 0.2s;">
            ...
            <div id="sortMenuContent" class="sort-menu-content" style="display: none; position: absolute; top: 100%; left: 0; margin-top: 8px; background: white; min-width: 220px; border-radius: 16px; box-shadow: 0 10px 25px -5px rgba(0,0,0,0.1), 0 8px 10px -6px rgba(0,0,0,0.02); border: 1px solid #e2e8f0; z-index: 1000; overflow: hidden;">
                <div class="sort-option" onclick="applySort('date_desc')" data-sort="date_desc">
                    <i class="fas fa-calendar-alt"></i> Date (plus récent)
                </div>
                <div class="sort-option" onclick="applySort('date_asc')" data-sort="date_asc">
                    <i class="fas fa-calendar-alt"></i> Date (plus ancien)
                </div>
                <div style="height: 1px; background: #e2e8f0; margin: 4px 0;"></div>
                <div class="sort-option" onclick="applySort('likes_desc')" data-sort="likes_desc">
                    <i class="fas fa-heart" style="color: #ef4444;"></i> Plus aimés
                </div>
                <div class="sort-option" onclick="applySort('likes_asc')" data-sort="likes_asc">
                    <i class="fas fa-heart" style="color: #ef4444;"></i> Moins aimés
                </div>
                <div style="height: 1px; background: #e2e8f0; margin: 4px 0;"></div>
                <div class="sort-option" onclick="applySort('length_desc')" data-sort="length_desc">
                    <i class="fas fa-align-left"></i> Plus longs
                </div>
                <div class="sort-option" onclick="applySort('length_asc')" data-sort="length_asc">
...
<script>
    function toggleSortMenu() {
        const menu = document.getElementById('sortMenuContent');
        const chevron = document.querySelector('.btn-sort .fa-chevron-down');
        
        if (menu.style.display === 'none' || menu.style.display === '') {
            // Fermer tous les autres menus
            document.querySelectorAll('.sort-menu-content').forEach(m => m.style.display = 'none');
    ...
    function applySort(sortType) {
        // Mettre à jour l'affichage actif dans le menu
        ...
        currentUrl.searchParams.set('order', sortType);
        currentUrl.searchParams.set('page', '1');
        window.location.href = currentUrl.toString();
    }
    
    // Surbrillance de l'option active au chargement
    document.addEventListener('DOMContentLoaded', function() {
        const currentOrder = new URLSearchParams(window.location.search).get('order') || 'date_desc';
        document.querySelectorAll('.sort-option').forEach(opt => {
            if (opt.getAttribute('data-sort') === currentOrder) {
                opt.classList.add('active');
            }
        });
    });
        ...
        <div style="width: calc(33.333% - 17px); min-width: 280px;">
            <a href="../Backoffice/showpost.php?id=<?= $post->getIdPost() ?>" style="text-decoration: none; display: block;">
                <div class="card post-card" style="padding: 14px; background: white; border-radius: 18px; box-shadow: 0 6px 20px rgba(0,0,0,0.08); transition: all 0.3s; height: 100%; display: flex; flex-direction: column;">
                    ...
                    <!-- Contenu principal -->
                    <div style="flex: 1;">
<!-- Post meta - en haut avec avatar et nom -->
<div style="display: flex; align-items: center; gap: 12px; margin-bottom: 12px;">
    <!-- Avatar avec la classe CSS -->
    <div class="avatar-css avatar-<?= htmlspecialchars($post->getUserAvatar()) ?>"
         style="width: 36px; height: 36px; border-radius: 50%; background-size: cover; background-position: center; display: flex; align-items: center; justify-content: center; color: white; font-weight: bold; background: linear-gradient(135deg, #0ea5e9, #3b82f6);">
    </div>
    <div>
        <!-- Nom de l'auteur -->
        ...
        <div class="post-date" style="font-size: 0.7rem; color: #64748b;">
            ...
            <?= (new DateTime($post->getDatePost()))->format('d/m/Y à H:i') ?>
                    ...
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-top: auto; padding-top: 15px; gap: 8px; border-top: 1px solid #f1f5f9;">
                        ...
                        <!-- Boutons actions -->
                        <!-- Boutons actions - seulement pour admin ou propriétaire -->
<?php 
