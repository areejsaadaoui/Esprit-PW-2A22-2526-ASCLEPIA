...
if (isset($_GET['export']) && $_GET['export'] === 'excel') {
    ...
    header('Content-Disposition: attachment; filename="contrats_assurance_' . $id_assurance . '_' . date('Ymd_His') . '.xls"');
    ...
    echo "<tr><td colspan='7' style='background:#0ea5e9;color:#fff;font-size:18px;font-weight:bold;padding:12px 14px;'>ASCLEPIA - Export Contrats par Assurance</td></tr>";
    echo "<tr><td colspan='7' style='padding:8px 14px;color:#475569;font-size:12px;'>Assurance: " . htmlspecialchars($assurance['nom_assurance'] ?? 'N/A') . " | Date export: " . date('d/m/Y H:i') . " | Total: " . count($contrats) . " contrat(s)</td></tr>";
    ...
    foreach ($contrats as $c) {
        ...
        echo "<tr style='background:{$bg};'>
                ...
                <td style='border:1px solid #e2e8f0;padding:7px;'>" . htmlspecialchars(!empty($c['type_assurance']) ? $c['type_assurance'] : ($assurance['TYPE'] ?? 'N/A')) . "</td>
...
$montantsParMois = [];
foreach ($contrats as $c) {
    $type = !empty($c['type_assurance']) ? $c['type_assurance'] : ($assurance['TYPE'] ?? 'N/A');
    $typeAssurance[] = $type;
    ...
    <div class="main-content">
        <div class="topbar">
            ...
            <div class="topbar-right">
                <a href="?id=<?= $id_assurance ?>&export=excel" class="btn btn-outline btn-sm">
        ...
        <div class="page-content">
                ...
                <table class="table">
                    ...
                    <tbody id="contratsTableBody">
                        ...
                        <tr class="contrat-row"
                            ...
                            data-search="<?= strtolower(htmlspecialchars($c['id_contrat'] . ' ' . $c['nom_assurance'] . ' ' . $c['date_d'] . ' ' . ($c['date_f'] ?? '') . ' ' . $c['montant'])) ?>"
            ...
            <div class="pagination">
                <?php if ($currentPage > 1): ?>
                    <a href="?id=<?= $id_assurance ?>&page=<?= $currentPage - 1 ?>"><i class="fa-solid fa-chevron-left"></i></a>
                <?php else: ?>
                ...
                <?php for ($i = 1; $i <= $totalPages; $i++): ?>
                    ...
                    <?php else: ?>
                        <a href="?id=<?= $id_assurance ?>&page=<?= $i ?>"><?= $i ?></a>
                    <?php endif; ?>
                <?php endfor; ?>
                <?php if ($currentPage < $totalPages): ?>
                    <a href="?id=<?= $id_assurance ?>&page=<?= $currentPage + 1 ?>"><i class="fa-solid fa-chevron-right"></i></a>
                <?php else: ?>
...
<script>
    ...
    function filtrerContrats() {
        if (!searchInput || !statutFilter) return;
        ...
        rows.forEach(function(row) {
            ...
            var matchSearch = search === '' || searchData.indexOf(search) !== -1;
