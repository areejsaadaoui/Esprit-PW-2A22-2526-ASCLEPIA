...
$isOwner = isset($_SESSION['user_id']) && $post->getIdUtilisateur() == $_SESSION['user_id'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['contenu']) && !empty($_POST['contenu'])) {
        ...
        if (empty($error)) {
            // Créer un nouvel objet Post avec les données modifiées
            ...
            // Appeler la méthode updatePost avec l'objet Post et l'ID
            $result = $postC->updatePost($updatedPost, $id_post);
            
            if ($result) {
                $success = "Post modifié avec succès !";
                header('Location: ../Frontoffice/postList.php?success=updated');
